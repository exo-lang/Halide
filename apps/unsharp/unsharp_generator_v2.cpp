...
class Unsharp_v2 : public Halide::Generator<Unsharp_v2> {
public:
    ...
    void generate() {
        ...
        // This version of the unsharp masking algorithm assumes
        // that the repeat_edge boundary condition has already been
        ...
        blur_x(x, y) = (kernel(0) * blur_y(x + 3, y) +
                        kernel(1) * (blur_y(x + 2, y) +
                                     blur_y(x + 4, y)) +
                        kernel(2) * (blur_y(x + 1, y) +
                                     blur_y(x + 5, y)) +
                        kernel(3) * (blur_y(x + 0, y) +
                                     blur_y(x + 6, y)));
...
HALIDE_REGISTER_GENERATOR(Unsharp_v2, unsharp_v2)
