            ...
            if (y_i == 0) {
                for (int_fast32_t yi = 0; yi < 6; yi++) {
                    for (int_fast32_t x = 0; x < 6 + W; x++) {
                        gray[yi * (6 + W) + x] = rgb_to_gray[0] * input[(yi + 32 * y) * (W + 6) + x] + rgb_to_gray[1] * input[(H + 6) * (W + 6) + (yi + 32 * y) * (W + 6) + x] + rgb_to_gray[2] * input[2 * (H + 6) * (W + 6) + (yi + 32 * y) * (W + 6) + x];
                    }
                }
            }
            for (int_fast32_t x = 0; x < 6 + W; x++) {
                gray[(6 + y_i) * (6 + W) + x] = rgb_to_gray[0] * input[(6 + y_i + 32 * y) * (W + 6) + x] + rgb_to_gray[1] * input[(H + 6) * (W + 6) + (6 + y_i + 32 * y) * (W + 6) + x] + rgb_to_gray[2] * input[2 * (H + 6) * (W + 6) + (6 + y_i + 32 * y) * (W + 6) + x];
            }
            for (int_fast32_t x = 0; x < 6 + W; x++) {
                blur_y[x] = kernel[0] * gray[(3 + y_i) * (6 + W) + x] + kernel[1] * (gray[(2 + y_i) * (6 + W) + x] + gray[(4 + y_i) * (6 + W) + x]) + kernel[2] * (gray[(1 + y_i) * (6 + W) + x] + gray[(5 + y_i) * (6 + W) + x]) + kernel[3] * (gray[y_i * (6 + W) + x] + gray[(6 + y_i) * (6 + W) + x]);
            }
            for (int_fast32_t x = 0; x < W; x++) {
                ratio[x] = (two[0] * gray[(3 + y_i) * (6 + W) + 3 + x] - (kernel[0] * blur_y[3 + x] + kernel[1] * (blur_y[2 + x] + blur_y[4 + x]) + kernel[2] * (blur_y[1 + x] + blur_y[5 + x]) + kernel[3] * (blur_y[x] + blur_y[6 + x]))) / gray[(3 + y_i) * (6 + W) + 3 + x];
            }
            for (int_fast32_t c = 0; c < 3; c++) {
                for (int_fast32_t x = 0; x < W; x++) {
                    output[c * H * W + (y_i + 32 * y) * W + x] = ratio[x] * input[c * (H + 6) * (W + 6) + (3 + y_i + 32 * y) * (W + 6) + 3 + x];
                }
            }
        }
        free(blur_y);
...
void exo_unsharp_base(void *ctxt, int_fast32_t W, int_fast32_t H, float *output, const float *input) {
    ...
    for (int_fast32_t y = 0; y < H + 6; y++) {
        for (int_fast32_t x = 0; x < W + 6; x++) {
            gray[y * (W + 6) + x] = rgb_to_gray[0] * input[y * (W + 6) + x] + rgb_to_gray[1] * input[(H + 6) * (W + 6) + y * (W + 6) + x] + rgb_to_gray[2] * input[2 * (H + 6) * (W + 6) + y * (W + 6) + x];
        }
    }
    free(rgb_to_gray);
    ...
    for (int_fast32_t y = 0; y < ((H) / (32)); y++) {
        ...
        for (int_fast32_t y_i = 0; y_i < 32; y_i++) {
            ...
            for (int_fast32_t xo = 0; xo < ((13 + W) / (8)) - 1; xo++) {
                ...
                var18 = _mm256_loadu_ps(&input[(H + 6) * (W + 6) + (6 + y_i + 32 * y) * (W + 6) + 8 * xo]);
                var16 = _mm256_mul_ps(var17, var18);
                var12 = _mm256_add_ps(var13, var16);
                var20 = _mm256_broadcast_ss(&rgb_to_gray[2]);
                var21 = _mm256_loadu_ps(&input[(2) * ((H + 6) * (W + 6)) + (6 + y_i + 32 * y) * (W + 6) + 8 * xo]);
            ...
            for (int_fast32_t xo = ((13 + W) / (8)) - 1; xo < ((13 + W) / (8)); xo++) {
...

                {
                    ...
                    __m256i prefix = _mm256_set1_epi32((6 - 8 * xo + W));
                    __m256i cmp = _mm256_cmpgt_epi32(prefix, indices);
                    var15 = _mm256_maskload_ps(&input[(6 + y_i + 32 * y) * (W + 6) + 8 * xo], cmp);
                }
...

                {
                    ...
                    __m256i prefix = _mm256_set1_epi32((6 - 8 * xo + W));
                    __m256i cmp = _mm256_cmpgt_epi32(prefix, indices);
                    var18 = _mm256_maskload_ps(&input[(H + 6) * (W + 6) + (6 + y_i + 32 * y) * (W + 6) + 8 * xo], cmp);
                }
...

                {
                    ...
                    __m256i prefix = _mm256_set1_epi32((6 - 8 * xo + W));
                    __m256i cmp = _mm256_cmpgt_epi32(prefix, indices);
                    var21 = _mm256_maskload_ps(&input[(2) * ((H + 6) * (W + 6)) + (6 + y_i + 32 * y) * (W + 6) + 8 * xo], cmp);
                }
            ...
            for (int_fast32_t xo = ((7 + W) / (8)) - 1; xo < ((7 + W) / (8)); xo++) {
...

                {
                    ...
                    __m256i prefix = _mm256_set1_epi32((-(8 * xo) + W));
                    __m256i cmp = _mm256_cmpgt_epi32(prefix, indices);
                    var47 = _mm256_maskload_ps(&gray[(3 + y_i) * (6 + W) + 3 + 8 * xo], cmp);
                }
...

                {
                    ...
                    __m256i prefix = _mm256_set1_epi32((-(8 * xo) + W));
                    __m256i cmp = _mm256_cmpgt_epi32(prefix, indices);
                    var69 = _mm256_maskload_ps(&gray[(3 + y_i) * (6 + W) + 3 + 8 * xo], cmp);
                }
            ...
            for (int_fast32_t c = 0; c < 3; c++) {
                for (int_fast32_t xo = 0; xo < ((W + 7) / (8)) - 1; xo++) {
                    ...
                    var72 = _mm256_loadu_ps(&input[(c) * ((H + 6) * (W + 6)) + (32 * y + y_i + 3) * (W + 6) + 8 * xo + 3]);
                    var70 = _mm256_mul_ps(var71, var72);
                    _mm256_storeu_ps(&output[(c) * (H * W) + (y_i + 32 * y) * W + 8 * xo], var70);
                }
                for (int_fast32_t xo = ((W + 7) / (8)) - 1; xo < ((7 + W) / (8)); xo++) {
...

                    {
                        ...
                        __m256i prefix = _mm256_set1_epi32((-8 * xo + W + 0));
                        __m256i cmp = _mm256_cmpgt_epi32(prefix, indices);
                        var72 = _mm256_maskload_ps(&input[(c) * ((H + 6) * (W + 6)) + (y_i + 32 * y + 3) * (W + 6) + 8 * xo + 3], cmp);
                    }
...

                    {
                        ...
                        __m256i prefix = _mm256_set1_epi32((-8 * xo + W + 0));
                        __m256i cmp = _mm256_cmpgt_epi32(prefix, indices);
                        _mm256_maskstore_ps(&output[(c) * (H * W) + (y_i + 32 * y) * W + 8 * xo], cmp, var70);
                    }
                }
            }
        }
